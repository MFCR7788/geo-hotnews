...
interface User {
  ...
  isBanned: boolean;
  createdAt: string;
  _count: { keywords: number; notifications: number };
}
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <motion.div
        ...
        className="bg-[#0a0a1a] border border-slate-800 rounded-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl"
      >
        {/* 头部 */}
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={u.id} className="border-b border-slate-800/50 hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <p className="text-slate-400 text-xs">{u._count.keywords} 关键词</p>
                        <p className="text-slate-500 text-xs">{u._count.notifications} 通知</p>
                      </td>
                      <td className="px-4 py-3">
