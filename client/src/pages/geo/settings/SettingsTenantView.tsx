      ...
      <div className="rounded-2xl border p-6" style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-default)' }}>
        <div className="space-y-6">
          {[
            ...
            <div key={field.key}>
              <label className="block text-base font-bold text-gray-900 mb-2">{field.label}</label>
              <input
                value={(form as any)[field.key]}
                disabled={field.key === 'name'}
                placeholder={field.placeholder}
                className={`w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/20 transition-all ${
                  field.key === 'name' 
          ...
          <div className="pt-4 border-t" style={{ borderColor: 'var(--border-subtle)' }}>
            <button
              onClick={saveTenant}
              disabled={saving}
              className="px-6 py-2.5 rounded-xl disabled:opacity-60 transition-colors"
